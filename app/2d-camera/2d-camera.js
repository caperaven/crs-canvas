...
export default class FaceSelect extends crsbinding.classes.ViewBase {
    ...
    async addMeshes() {
        ...
        const meshes = [];
        meshes.push(await crs.call("gfx_geometry", "add", { element:  this.canvas, data: "icons/home", position: {x: -1, y: 1}, color: "#ff0090" }));
        meshes.push(await crs.call("gfx_geometry", "add", { element:  this.canvas, data: "flowchart/documents", position: {x: 1, y:2}, color: "#ff9000" }));
        meshes.push(await crs.call("gfx_geometry", "add", { element:  this.canvas, data: "floorplan/fire_hose", position: {x: 0, y: 0, z:-0.1}, color: "#9000ff" }));
    ...
    async createPlane() {
        ...
        this.bgPlane = BABYLON.MeshBuilder.CreatePlane("plane", {size: 200, position: {z:-0.0001}}, this.canvas.__layers[0]);
